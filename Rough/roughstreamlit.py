...
def load_conversion_data():
    ...
    return {
        ...
        "Fuel Economy": {
            ...
            # Changed to indicate special handling needed
            "Liter per 100 kilometers": "SPECIAL"
        },
        "Plane Angle": {
            "Degree": 1,
            "Radian": 57.2958,
        ...
        if from_unit == "Liter per 100 kilometers" and to_unit == "Miles per gallon (US)":
            ...
            if from_unit in conversion_data[category] and to_unit in conversion_data[category]:
                if isinstance(conversion_data[category][from_unit], (int, float)) and isinstance(conversion_data[category][to_unit], (int, float)):
                    factor = conversion_data[category][from_unit] / \
                        conversion_data[category][to_unit]
...
# Main App UI
st.markdown("<h1 class='main-header'>Multi-Unit Converter</h1>",
            unsafe_allow_html=True)
st.markdown("<p class='sub-header'>Convert between different units of measurement</p>",
            unsafe_allow_html=True)
...
with col1:
    ...
    category = st.selectbox("Select category", list(
        conversion_data.keys()), label_visibility="collapsed")
...
with col2:
    ...
    with col_from:
        from_unit = st.selectbox("From", list(
            conversion_data[category].keys()), key="from_unit")
        from_value = st.number_input(
            "Enter value", value=1.0, format="%.8f", key="from_value")

    with col_equal:
        st.markdown(
            "<div style='text-align: center; font-size: 2rem; margin-top: 1.7rem;'>=</div>", unsafe_allow_html=True)

    with col_to:
        to_unit = st.selectbox("To", list(
            conversion_data[category].keys()), key="to_unit")
        ...
        if isinstance(to_value, float):
            st.markdown(
                f"<div class='value-display'>{to_value:.{precision}f}</div>", unsafe_allow_html=True)
        else:
            st.markdown(
                f"<div class='value-display'>{to_value}</div>", unsafe_allow_html=True)

    formula = get_formula(from_unit, to_unit, category, from_value, to_value)
    st.markdown(
        f"<div class='formula-box'><strong>Formula:</strong> {formula}</div>", unsafe_allow_html=True)

    detailed_explanation = get_conversion_details(
        from_unit, to_unit, category, from_value, to_value)
    st.markdown(
        f"<div class='conversion-details'><strong>Details:</strong>\n{detailed_explanation}</div>", unsafe_allow_html=True)

    st.markdown("</div>", unsafe_allow_html=True)

# Footer
st.markdown("<div class='footer'>Developed by Riaz Hussain | © 2025 Multi-Unit Converter</div>",
            unsafe_allow_html=True)
